...
def _report_prompt(is_forged: bool) -> str:
    if is_forged:
        return (
            "You are an image forgery analysis expert. The input contains two images: "
            "the first is the original image, and the second is a suspicious-region mask "
            "generated by a detection model. White areas in the mask indicate likely tampered regions. "
            "Write a concise professional report in English. First state whether the image is suspected "
            "to be forged. Then describe the approximate location of the suspicious regions, visible "
            "anomalies, possible manipulation type, and why these regions may look inconsistent in "
            "texture, edges, lighting, text layout, object boundaries, or semantic structure. "
            "Do not invent exact coordinates, text, objects, or details that are not visible."
        )
    return (
        "You are an image forgery analysis expert. The detection model classified this image as authentic. "
        "Write a concise professional report in English explaining why no obvious manipulation signs are "
        "detected. Discuss lighting consistency, edge and texture continuity, plausible object or text "
        "arrangement, perspective, and overall semantic coherence. Avoid inventing details that are not visible."
    )
